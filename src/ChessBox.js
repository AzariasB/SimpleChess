...
import { Tools, COLOR, P_HEX } from './tools';

export const ChessBox = Backbone.Model.extend({
	defaults          : {
		PRESENT    : 0,
		WHITEBEGUN : [],
		BLACKBEGUN : [],
	...
	currentIsBegun    : function() {
		return this.getCurrent() && !_.isEmpty(this.getBegun(Tools.getInvertColor(this.getCurrent())));
	},
	/**
	...
	isBegunBy         : function(begunner) {
		...
		return Tools.isWhite(color)
			? !_.isEmpty(this.getWhiteBegun())
	...
	kingCanComeHere   : function(king) {
		...
		return (
			...
			(!this.isEmpty() &&
				!Tools.sameColor(this.getCurrent(), color) &&
				Tools.ifWhiteElseIfBlack(king, _.isEmpty(this.getBlackBegun(), _.isEmpty(this.getWhiteBegun()))))
		);
	},
	/**
