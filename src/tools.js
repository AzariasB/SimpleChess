...
export const Tools = {
	...
	containPinOrBegun     : function(begunPin) {
		return begunPin && (begunPin.begun || begunPin.pin) && (!_.isEmpty(begunPin.begun) || !_.isEmpty(begunPin.pin));
	},
	...
	isOnHisLastLine       : function(piece, position) {
		return this.ifWhiteElseIfBlack(piece, position >= 56 && position <= 63, position >= 0 && position <= 7);
	},
	ifWhiteElseIfBlack    : function(piece, returnWhite, returnBlack) {
		var color = this.getPieceColor(parseInt(piece));
		return color === COLOR.WHITE ? returnWhite : color === COLOR.BLACK ? returnBlack : undefined;
	},
