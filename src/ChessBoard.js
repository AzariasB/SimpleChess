...
export const ChessBoard = Backbone.Collection.extend({
	...
	kingConditions        : function(king, index, incrementation, color) {
		...
		var ennemies = {},
			invertColor;
		if (
			Tools.isValidIndex(dIndex) &&
			!(
				(Tools.isAtLeftBorder(index) && Tools.turnLeft(incrementation)) ||
	...
	updateRook            : function(rookPiece, index) {
		...
		_.each(directions, function(dir) {
			...
			if (!_.isEmpty(res) && res && res.begun && res.pin && (!_.isEmpty(res.begun) || !_.isEmpty(res.pin))) {
				iGotTheKing = res;
			}
		});
		return iGotTheKing;
	...
	updatePawn            : function(pawnPiece, index) {
		...
		var color = Tools.getPieceColor(pawnPiece),
			...
			pDroite = 9;
		if (color === COLOR.BLACK) {
			(pDevant = -pDevant), (pDroite = -pDroite), (pGauche = -pGauche);
		}
	...
	checkDiagonal         : function(piece, indexStart, incrementation) {
		...
		if (
			!(
				(Tools.isAtRightBorder(indexStart) && Tools.turnRight(incrementation)) ||
			...
			while (!_.isUndefined(direction) && dIndex >= 0 && dIndex <= 64) {
				...
				if (
					(Tools.isAtRightBorder(direction.id) && Tools.turnRight(incrementation)) ||
