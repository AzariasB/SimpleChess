	...
	proposeEnPassant : function(chess, event) {
		...
		if (Tools.sameType(pieceSelect, P_HEX.PAWN)) {
			...
			if (lastMove) {
				...
				if (
					Tools.sameType(P_HEX.PAWN, lastMove.get('piece')) &&
					difference === 16 &&
	...
	validEnPassant   : function(chess, event, move) {
		...
		if (Tools.sameType(move.get('piece'), P_HEX.PAWN) && !move.get('eat')) {
			...
			if (lastMove) {
				...
				if (Tools.sameColumn(lastTo, now) && !Tools.sameColumn(lastTo, origin)) {
...
export const castling = {
	...
	proposeCastling   : function(chess, event) {
		...
		if (
			Tools.sameType(selected, P_HEX.KING) &&
			!chess.moves.moves.myKingMoved(color) &&
			!event.case.currentIsBegun()
		) {
			var chosenColor = Tools.ifWhiteElseIfBlack(
				color,
				this.positionsCastling.white,
				this.positionsCastling.black
			);
	...
	confirmCastling   : function(chess, event, move) {
		...
		if (Tools.sameType(moved, P_HEX.KING) && Math.abs(difference) >= 2) {
			var rookOrigin, rookDestination, moveName;
			if (difference > 0) {
				//grand roque
				rookOrigin = to + 2;
...
export const mat = {
	...
	checkForMat          : function(chess, event, move) {
		...
		if (
			chess.board.chessBoard.myKingIsChess(color) &&
